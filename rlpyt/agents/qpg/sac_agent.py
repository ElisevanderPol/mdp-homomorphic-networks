...
class SacAgent(BaseAgent):
    ...
    def __init__(
            self,
            ModelCls=PiMlpModel,  # Pi model.    # just change this to equiv.?
            QModelCls=QofMuMlpModel,     # just change this too inv.?
        ...
        if model_kwargs is None:
            model_kwargs = dict(hidden_sizes=[64, 64]) #, steerable=steerable)
        if q_model_kwargs is None:
            q_model_kwargs = dict(hidden_sizes=[64, 64]) #, steerable=steerable)
        if v_model_kwargs is None:
            v_model_kwargs = dict(hidden_sizes=[64, 64]) #, steerable=steerable)
